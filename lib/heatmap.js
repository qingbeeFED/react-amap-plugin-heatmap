...
var Heatmap = function (_React$Component) {
    ...
    if (typeof window !== 'undefined') {
      if (!props.__map__) {
        ...
        _this.resolveHeatmap(props).then(function () {
          ...
          if ('dataSet' in props) {
            _this.createAMap(props.dataSet, props.mapType);
          }
        });
      }
    }
  ...
  _createClass(Heatmap, [{
    ...
    key: 'createAMap',
    value: function createAMap(dataSet) {
      var mapType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';

      var mapTypes = ['baidu', 'gps', 'mapbar'];
      ...
      AMap.convertFrom(points, mapType, function (status, result) {
        if (result.info === 'ok') {
          ...
          var rData = lnglats.map(function (item, index) {
            return _extends({}, dataSet.data[index], {
              lng: item.getLng(),
              lat: item.getLat()
            });
          });
          this.heatmap.setDataSet(_extends({}, dataSet, {
            data: rData
          }));
        }
      });
